...
public class FuelCenter implements Serializable {
   ...
   public static void main(String[] args) {
      ...
      while (true) {
         ...
         if (option.equals("100") || option.equalsIgnoreCase("VFQ")) {
            ...
            fuelStock = removeServedCustomer(fuelQueues, fuelStock); //Remove served customer method called.
            ...
            viewFuelStock(fuelStock);     //viewFuelStock method called to display remaining fuel stock.
   ...
   /**
    * This method is used to add a customer to a fuel queue. If all queues are full the customer will be added to the waiting queue from this method.
    *
    * @param queueRef FuelQueue object array.
   ...
   public static int addCustomer(FuelQueue[] queueRef, int stock) {
      ...
      do {
         ...
         //Checks if all queues are full. If so tries to add customer to the waiting queue.
         if (fullQueues == numberOfPumps) {

            //Check if waiting queue is full. If waiting queue is full user is given instructions to return to menu.
            if (waitingQueue.isFull()) {
               System.out.println("\nWARNING!!! Waiting Queue is full! Can not add more customers\n");
               System.out.println("Press enter to return to the menu and serve customers to add more customers!\n");

            } else {
               //Display message to show user customer was added to waiting queue.
               System.out.println("\nWARNING!!! All queues are full! Customer will be added to the waiting queue");

               //enQueue method called from WaitingQueue class. Passenger object passed as a parameter to add new customer in waiting queue.
               waitingQueue.enQueue(new Passenger(firstName, lastName, vehicleNumber, fuelAmount));
               System.out.println("\n" + firstName + " " + lastName + " was successfully added to the waiting queue!\n");
            }
            ...
            for (int i = 0; i < maxQueueSize; i++) {    //Looping through the columns of the fuelQueue array
               if (queueRef[pump].getCustomerDetails(i).getFirstName().equals("empty")) { //Checks if slot is empty to add and replace
                  ...
                  //Adds customer using setters from Passenger class to pump assigned from line 210.
      ...
      do {
         ...
         if (queueRef[pump - 1].getCustomerDetails(slot - 1).getFirstName().equals("empty")) {
            ...
            if (WaitingQueue.isEmpty()) {
               System.out.println("Waiting Queue is Empty! Therefore no customers were added to the queue");

               //If waiting queue is empty assigns last index of array list with a new Passenger object to avoid printing null.
               queueRef[pump - 1].assignCustomer(new Passenger("empty", "empty", "empty", 0));

            } else {
               //If there is a customer in waiting queue added to the fuel queue based on which queue customer was removed from
               Passenger waitingCustomer = waitingQueue.deQueue();
               queueRef[pump - 1].assignCustomer(waitingCustomer);
               System.out.println(waitingCustomer.getFullName() + " from the waiting queue was added to fuel queue " + pump);
               stock -= waitingCustomer.getFuelAmount(); //Reducing stock from waiting queue customer
            }
            System.out.print("\nEnter 'R' to remove another customer from a specific location or press enter to return to the menu: ");
   ...
   /**
    * This method is to remove a served customer. If there is a customer in the waiting queue that customer will be automatically
   ...
   public static int removeServedCustomer(FuelQueue[] queueRef, int stock) {
      ...
      do {
         ...
         if (queueRef[pump - 1].getCustomerDetails(0).getFirstName().equals("empty")) {
            ...
            System.out.print("\nEnter 'S' to remove another served customer or press enter to return to the menu: ");
   ...
   public static void viewFuelStock(int stock) {
      System.out.println(stock + " Litres of Fuel remaining"); //Displays remaining fuel stock.
   }
